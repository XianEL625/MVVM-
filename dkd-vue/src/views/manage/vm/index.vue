  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="vmRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="供货时间" prop="innerCode" v-if="form.innerCode!=null">
          <span>{{ parseTime(form.lastSupplyTime,"{y}-{m}-{d} {h}:{i}:{s}") }}</span>
        </el-form-item>
        <el-form-item label="设别类型" prop="innerCode" v-if="form.innerCode!=null">
          <div v-for="item in vmTypeList" :key="item.id">
            <span v-if="item.id==form.vmTypeId">{{ item.name }}</span>
          </div>
        </el-form-item>
        <el-form-item label="设备容量" prop="innerCode" v-if="form.innerCode!=null">
        ...
        <el-form-item label="选择型号" prop="vmTypeId" v-if="form.innerCode==null">
          ...
          <el-select v-model="form.vmTypeId" placeholder="请选择设备型号">
           <el-option v-for="item in vmTypeList" :key="item.id" :label="item.name" :value="item.id">{{ item.name }}</el-option> 
          </el-select> 
        </el-form-item>
        <el-form-item label="选择点位" prop="nodeId">
          <!-- <el-input v-model="form.nodeId" placeholder="请输入点位" /> -->
           ...
           <el-option v-for="item in nodeList" :key="item.id" :label="item.nodeName" :value="item.id">{{ item.nodeName }}</el-option> 
          </el-select>
        </el-form-item>
