...
<div class="card" style="margin-bottom: 20px; font-size: 18px;">
  ...
  <div class="demo-collapse class" style="margin-bottom: 5px;">  
    ...
    <el-collapse v-model="data.activeName" accordion>  
      <el-collapse-item v-for="item in data.form" :key="item.id" :title="item.title" :name="item.id" >  
        <div style="padding: 20px" >{{item.context}}</div>
      </el-collapse-item>  
    </el-collapse>  
  </div>  
</template>  
...
const findNotice = async () => {  
  try {  
    const res = await request.get("/Inotice/getAll");  
    if (res.code === "200") {  
      data.form = res.data;  
