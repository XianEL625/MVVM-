...
<img v-if="scope.row.avatar" :src="scope.row.avatar" alt="" style="width: 40px;display:block;height: 40px;border-radius: 50%;">
    ...
    <el-table-column prop="registrationCount" label="就诊数量" />
    <el-table-column prop="tel" label="电话" show-overflow-tooltip/>
    <el-table-column prop="email" label="邮箱" show-overflow-tooltip/>
    <el-table-column prop="department" label="科室" />
    <el-table-column prop="schedule" label="值班" show-overflow-tooltip/>
    <el-table-column prop="hireDate" label="入职时间" show-overflow-tooltip/>
  ...
  <el-button type="primary">上传头像</el-button>
      ...
      <el-form-item label="挂号费" label-width="80px">
        <el-input v-model="data.form.registrationFee" autocomplete="off" />
      </el-form-item>
      <el-form-item label="就诊数量" label-width="80px">
        <el-input v-model="data.form.registrationCount" autocomplete="off" />
      </el-form-item>
    ...
    >
      ...
      <el-form-item label="值班" label-width="80px">
        <el-input v-model="data.form.schedule" autocomplete="off" />
      </el-form-item>
      <el-form-item label="入职时间" label-width="80px">
        <el-date-picker
        v-model="data.form.hireDate2"
        type="date"
        placeholder="选一天"
        :shortcuts="shortcuts"
      />
...
const reset=()=>{
 ...
 const findDepartment=()=>{
  request.get('/departments/selectAll').then(res=>{
    if(res.code==="200"){
      data.departmentObjs=res.data
    }else{
...
const save=()=>{
 ...
 data.form.id?update():add()
  ...
  const add = () => {  
    request.post('/doctors/add',JSON.stringify(data.form)).then(res =>{
      if(res.code="200"){
